...
export function solve1(data: number[][]): number {
  ...
  return valMap.entries().filter(([_, val]) => val === 0).map(([key, _]) =>
    Array.from(transitiveClosure.get(key)!.values()).filter((key) =>
      valMap.get(key)! === 9
    ).length
  ).reduce(
    (acc, x) => acc + x,
...
export function solve2(data: number[][]): number {
  ...
  const uniquePathCount = new Map<string, number>();

  const findUniquePathCount = (key: string) => {
    if (valMap.get(key)! === 9) return 1;

    if (uniquePathCount.has(key)) {
      return uniquePathCount.get(key)!;
    }

    let count = 0;
    for (const neighbor of adjMap.get(key)!) {
      count += findUniquePathCount(neighbor);
    }
    return count;
  };
